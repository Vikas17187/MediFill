...
// Medicine interaction database (simplified)
// In a real app, this would come from a medical API
const MEDICINE_INTERACTIONS: Record<string, string[]> = {
  ...
  'Potassium supplements': ['Lisinopril'],
  'Metformin': ['Alcohol'],
  'Levothyroxine': ['Calcium supplements', 'Iron supplements'],
  'Calcium supplements': ['Levothyroxine'],
...
export const MedicineProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const findInteractions = (medicineName: string): string[] => {
    ...
    // Find the key in the interactions database that matches the medicine name
    const interactionKey = Object.keys(MEDICINE_INTERACTIONS).find(
      key => key.toLowerCase() === normalizedName
    );
    
    if (!interactionKey) return [];
    
    // Get the list of medicines that interact with this one
    const interactingMedicines = MEDICINE_INTERACTIONS[interactionKey];
    
    // Find which of these interacting medicines the user is currently taking
    return medicines
      .filter(med => 
        interactingMedicines.some(
          interactingMed => med.name.toLowerCase() === interactingMed.toLowerCase()
        )
      )
      .map(med => med.name);
  };
        ...
        medicines.forEach(medicine2 => {
          if (medicine1.id !== medicine2.id) {
            ...
            if (!checkedPairs.has(pairKey) && !currentProcessedIds.has(interactionAlertId)) {
              ...
              const med1Interactions = MEDICINE_INTERACTIONS[medicine1.name] || [];
              if (med1Interactions.includes(medicine2.name)) {
                newAlerts.push({
